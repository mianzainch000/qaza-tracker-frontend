...
const History = () => {
    ...
    const filteredLogs = useMemo(() => {
        return logs.filter((log) => {
            const dateObj = new Date(log.date);
            const monthFull = dateObj.toLocaleString("default", { month: "long" }).toLowerCase();
            const monthShort = dateObj.toLocaleString("default", { month: "short" }).toLowerCase();
            const year = dateObj.getFullYear().toString();
            const search = searchTerm.toLowerCase().trim();
            const fullDateString = `${monthFull} ${monthShort} ${year}`;
            const searchWords = search.split(" ");
            return searchWords.every(word => fullDateString.includes(word));
        });
    }, [logs, searchTerm]);

    // Pagination calculations (Ab ye filteredLogs use karega)
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (res?.status === 200) {
                showAlert({ message: res?.data?.message || "Record deleted! ✅", type: "success" });
            ...
            <div className={styles.homeMainContainer}>
                <div className={styles.tableContainer}>
                    <div className={styles.header}>
                        ...
                        <HistoryFilter searchTerm={searchTerm} onSearchChange={(val) => { setSearchTerm(val); setCurrentPage(1); }} />

                        <div style={{ display: 'flex', justifyContent: 'center', marginTop: '10px' }}>
                            <button onClick={() => exportToPDF(filteredLogs)} className={styles.downloadBtn}>
                                📥 Export PDF
                            </button>
                        </div>
                    </div>
                ...
                {/* Pagination (filteredLogs check karke) */}
